  ...
  return (
    <div className="relative mx-auto aspect-square w-full max-w-[22rem] p-6">
      {accentSrc ? (
        <div
          aria-hidden="true"
          className="absolute right-0 top-0 h-[62%] w-[62%] rotate-6 overflow-hidden rounded-2xl border border-border/60 opacity-40 grayscale mix-blend-luminosity sm:opacity-50"
        >
          <Image
            src={accentSrc}
            alt={accentAlt}
        ...
        <div
          aria-hidden="true"
          className="absolute right-0 top-0 flex h-[62%] w-[62%] rotate-6 items-center justify-center rounded-2xl border border-dashed border-border/60 font-mono text-[10px] text-muted-foreground"
        >
          [[ supporting image ]]
        </div>
      )}
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative h-[68%] w-[68%] overflow-hidden rounded-2xl border-2 border-primary/70 bg-card shadow-[0_0_40px_-10px_hsl(var(--primary)/0.5)]">
          {avatarSrc ? (
            <Image
              src={avatarSrc}
              alt={avatarAlt}
